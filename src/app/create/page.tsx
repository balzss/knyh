...
export default function Create() {
  ...
  const [ingredientList, setIngredientList] = useState<IngredientItem[]>([{id: nanoid(), value: ''}])
  const [focusedIngredientId, setFocusedIngredientId] = useState<string>('')
  const ingredientInputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({})
  ...
  const handleIngredientItemChange = (changeEvent: React.ChangeEvent<HTMLInputElement>, itemId: string) => {
    const { value } = changeEvent.target
    const lastItem = ingredientList.findIndex(item => item.id === itemId) === ingredientList.length - 1
    setIngredientList(prevItems => {
      const updatedValues = prevItems.map(item => item.id === itemId ? { ...item, value, autoFocus: false } : item)
  ...
  const handleIngredientItemKeydown = (e: React.KeyboardEvent<HTMLInputElement>, itemId: string) => {
    const currentIndex = ingredientList.findIndex(item => item.id === itemId)

    if (e.key === 'Backspace' && ingredientList.find(item => item.id === itemId)?.value === '') {
      setIngredientList(prevItems => prevItems.filter((item) => {
      ...
      setIngredientList(prevItems => {
        ...
        return [...prevItems.slice(0, currentIndex + 1), {id: nanoid(), value: '', autoFocus: true}, ...prevItems.slice(currentIndex + 1)]
      })
    }
  }

  const handleRemoveIngredient = (itemId: string) => {
    setIngredientList(prevItems => prevItems.filter(item => item.id !== itemId))
  }
      ...
      <main className="w-full mt-16 mx-auto">
        <PageLayout>
          ...
          <div className="grid w-full items-center gap-2 mb-4">
            ...
            <DndContext onDragEnd={handleDragEnd} modifiers={[restrictToParentElementCustom]} id="dnd-context">
              <ul className={`grid gap-[0.5rem]`}>
                <SortableContext items={ingredientList} strategy={verticalListSortingStrategy}>
                  <AnimatePresence>
